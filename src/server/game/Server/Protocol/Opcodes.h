...
enum SessionStatus
{
    STATUS_AUTHED = 0,                   // 身份验证 Player authenticated (_player == NULL, m_playerRecentlyLogout = false or will be reset before handler call, m_GUID have garbage)
    STATUS_LOGGEDIN,                     // 已经登录 Player in game (_player != NULL, m_GUID == _player->GetGUID(), inWorld())
    STATUS_TRANSFER,                     // 地图传送 Player transferring to another map (_player != NULL, m_GUID == _player->GetGUID(), !inWorld())
    ...
    STATUS_NEVER,                        // Opcode not accepted from client (deprecated or server side only)客户端不接受操作码（已弃用或仅限服务器端）
    STATUS_UNHANDLED                     // Opcode not handled yet 操作码尚未处理
};
