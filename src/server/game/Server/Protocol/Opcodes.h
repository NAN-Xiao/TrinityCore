...
enum SessionStatus
{
    STATUS_AUTHED = 0, // 玩家认证（_player == NULL, m_playerRecentlyLogout = false或将在处理程序调用之前重置，m_GUID有垃圾）// Player authenticated (_player == NULL, m_playerRecentlyLogout = false or will be reset before handler call, m_GUID have garbage)
    STATUS_LOGGEDIN,   // 游戏中的玩家(_player != NULL, m_GUID == _player->GetGUID（), inWorld()） // Player in game (_player != NULL, m_GUID == _player->GetGUID(), inWorld())
    // STATUS_TRANSFER,                  //玩家转移到另一个地图(_player != NULL, m_GUID == _player->GetGUID（), !inWorld()） // Player transferring to another map (_player != NULL, m_GUID == _player->GetGUID(), !inWorld())
    STATUS_LOGGEDIN_OR_RECENTLY_LOGGOUT, //_player != NULL或_player == NULL && m_playerRecentlyLogout && m_playerLogout， m_GUID存储最后的_player guid)// _player != NULL or _player == NULL && m_playerRecentlyLogout && m_playerLogout, m_GUID store last _player guid)
    STATUS_NEVER,                        // 客户端不接受操作码（已弃用或仅限服务器端）// Opcode not accepted from client (deprecated or server side only)
    STATUS_UNHANDLED                     // 操作码尚未处理 // Opcode not handled yet
};
