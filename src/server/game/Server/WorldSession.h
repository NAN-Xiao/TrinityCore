...
public:
    ...
    // 如果包含的所有超链接都有效，则返回true
    // 可能会根据世界配置将玩家踢到false（处理程序应该中止）
     ...
     *@fn void WorldSession::SendAuctionCommandResult(uint32 auctionId, uint32 action, uint32 errorCode, uint32 bagError = 0)；
     *
     *@brief*@brief通知客户上次拍卖操作的结果。当玩家出价、创建或删除拍卖时调用
     *
     *@param auctionId相关的拍卖id
     *@param command执行的操作。
     *@param errorCode产生的错误代码。
     *@param bagError（可选）InventoryResult。
     */
