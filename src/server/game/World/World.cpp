...
//??????????m_worldLoopCounter?????,??????World???
//?????0,???????????????? 0?
//????,???????????World????? 32 ??????????m_worldLoopCounter,????? 0??????????????????World????????????????????????????
...
void World::Update(uint32 diff)
{
    ...
    /// 处理AHBot操作(=AHBot 通常指的是一种用于魔兽世界（World of Warcraft）游戏中的拍卖行机器人)
    ...
    /// 在更新sScriptReloadMgr之前，同步所有脚本及其id
...
{
    m_timers[WUPDATE_EVENTS].Reset(); // to give time for Update() to be processed //?Update()????
    uint32 nextGameEvent = sGameEventMgr->Update();
    m_timers[WUPDATE_EVENTS].SetInterval(nextGameEvent);
    m_timers[WUPDATE_EVENTS].Reset();
}
...
void World::UpdateWarModeRewardValues()
{
    ...
    if (std::any_of(warModeEnabledFaction.begin(), warModeEnabledFaction.end(), [](int64 val)
                    { return val != 0; }))
    {
