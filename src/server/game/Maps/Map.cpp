...
Map::Map(uint32 id, time_t expiry, uint32 InstanceId, Difficulty SpawnMode) : _creatureToMoveLock(false), _gameObjectsToMoveLock(false), _dynamicObjectsToMoveLock(false), _areaTriggersToMoveLock(false),
...
// player在登录完成后会把调用自己的getmap并把自己添加进来
    ...
    for (MapReference const &mapReference : m_mapRefManager)
    {
        if (worldStateTemplate && !worldStateTemplate->AreaIds.empty())
        {
            bool isInAllowedArea = std::any_of(worldStateTemplate->AreaIds.begin(), worldStateTemplate->AreaIds.end(),
                                               ...
                                               { return DB2Manager::IsInArea(playerAreaId, requiredAreaId); });
            if (!isInAllowedArea)
                continue;
        }
...
void Map::Update(uint32 t_diff)
{
    ...
    for (m_mapRefIter = m_mapRefManager.begin(); m_mapRefIter != m_mapRefManager.end(); ++m_mapRefIter)
    {
        ...
        // 如果玩家使用远视或心视，也要访问那个对象
        if (WorldObject *viewPoint = player->GetViewpoint())
            VisitNearbyCellsOf(viewPoint, grid_object_update, world_object_update);

        // Handle updates for creatures in combat with player and are more than 60 yards away
        // 处理生物在战斗中与玩家和超过60码远的更新
...
{
    if ((checks & LINEOFSIGHT_CHECK_VMAP) && !VMAP::VMapFactory::createOrGetVMapManager()->isInLineOfSight(PhasingHandler::GetTerrainMapId(phaseShift, GetId(), m_terrain.get(), x1, y1), x1, y1, z1, x2, y2, z2, ignoreFlags))
        return false;
...
void Map::DeleteRespawnInfo(RespawnInfo *info, CharacterDatabaseTransaction dbTrans)
{
    ...
    auto it = std::find_if(range.first, range.second, [info](RespawnInfoMap::value_type const &pair)
                           { return (pair.second == info); });
