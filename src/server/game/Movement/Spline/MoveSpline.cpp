...
#undef CHECK
    ...
    bool MoveSplineInitArgs::_checkPathLengths()
    {
        ...
        auto isValidPackedXYOffset = [](float coord) -> bool
        { return coord > MIN_XY_OFFSET && coord < MAX_XY_OFFSET; };
        auto isValidPackedZOffset = [](float coord) -> bool
        ...
        if (path.size() > 2)
        {
            ...
            for (uint32 i = 1; i < path.size() - 1; ++i)
            {
                ...
                if (!flags.UncompressedPath)
                    if (!isValidPackedXYOffset(middle.x - path[i].x) || !isValidPackedXYOffset(middle.y - path[i].y) || !isValidPackedZOffset(middle.z - path[i].z))
