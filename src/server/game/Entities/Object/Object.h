...
public:
    ...
    {
        if (IsItem())
            return reinterpret_cast<Item const *>(this);
        else
            return nullptr;
    }
    ...
    {
        if (IsSceneObject())
            return reinterpret_cast<SceneObject *>(this);
        else
            return nullptr;
    }
    ...
    {
        if (IsSceneObject())
            return reinterpret_cast<SceneObject const *>(this);
        else
            return nullptr;
    }
    ...
    {
        if (IsConversation())
            return reinterpret_cast<Conversation *>(this);
        else
            return nullptr;
    }
    ...
    {
        if (IsConversation())
            return reinterpret_cast<Conversation const *>(this);
        else
            return nullptr;
    }
...
private:
    ...
    {
        void operator()(Object *) const { /*noop - not managed*/ }
    };
...
{
protected:
    explicit WorldObject(bool isWorldObject); // 这里指的是它是否在网格对象列表或世界对象列表中//note: here it means if it is in grid object list or world object list
public:
    virtual ~WorldObject();
...
private:
    ...
    uint32 m_InstanceId; // 使用实例id复制映射 // in map copy with instance id
...
namespace Trinity
{
    ...
    // 基于到引用WorldObject的距离对WorldObject进行排序的二元谓词
