...
public:
    ...
    {
        if (on)
            SetPlayerFlag(PLAYER_FLAGS_DEVELOPER);
        else
            RemovePlayerFlag(PLAYER_FLAGS_DEVELOPER);
    }
    ...
    // 根据声明的语言和配置预定义的范围在常规聊天中处理所述消息。
    void Say(std::string_view text, Language language, WorldObject const * = nullptr) override;
    void Say(uint32 textId, WorldObject const *target = nullptr) override;
    /// Handles yelled message in regular chat based on declared language and in config pre-defined Range.
    // 根据声明的语言和配置预定义的范围处理常规聊天中的喊出消息。
...
public:
    ...
    // 如果玩家在室外PVP目标捕获中处于活动状态则返回true，否则返回false
    bool IsOutdoorPvPActive() const;

    /*********************************************************/
    /***           环境系统   ENVIROMENTAL SYSTEM           ***/
    /*********************************************************/
    ...
    void SetRestState(RestTypes type, PlayerRestState state)
    {
        SetUpdateFieldValue(m_values
                                ...
                                .ModifyValue(&UF::RestInfo::StateID),
                            state);
    }
...
protected:
    ...
    /*********************************************************/
    /***           战场    BATTLEGROUND SYSTEM             ***/
    /*********************************************************/

    /*
    this is an array of BG queues (BgTypeIDs) in which is player
    这是BG队列（bgtypeid）的数组，其中是player
    */
    ...
    // 我们只允许一个定时任务在同一时间活动。下面可以是简单的value而不是set。
    ...
    uint32 m_lastPotionId; // 在战斗中最后一次使用生命/法力药水，会阻止下一次使用// last used health/mana potion in combat, that block next potion use
...
// 将具有连续存储的自定义选项的容器转换为不依赖于容器的迭代器对
// 并且不强制在头文件中实现
